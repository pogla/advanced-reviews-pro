	...
	function( $ ) {
		...
		function CMB2ConditionalsInit( changeContext, conditionContext ) {
			...
			if ( 'undefined' === typeof conditionContext ) {
			  ...
			  */
			changeContext.on(
				'change', 'input, textarea, select', function( evt ) {
					...
					if ( dependants.length > 0 ) {
						...
						if ( 'checkbox' === elm.attr( 'type' ) ) {
							checkedValues = $( '[name="' + fieldName + '"]:checked' ).map(
								function() {
									return this.value;
								}
							).get();
					  ...
					  */
					conditionContext.on(
						'click', '.cmb-multicheck-toggle', function( evt ) {
					...
					conditionContext.on(
						'hide', '.cmb-row', function() {
							$( this ).children( '[data-conditional-required="required"]' ).each(
								function( i, e ) {
			...
			for ( loopI = 0; loopI < uniqueFormElms.length; loopI++ ) {
				...
				if ( 1 === formElms.length || ! formElms.is( ':checked' ) ) {
					  ...
					  */
					$( '.cmb2-wrap > .cmb2-metabox' ).on(
						'cmb2_add_row', function( evt, row ) {
							var rowFormElms,
		...
		function CMB2ConditionalsFindDependants( fieldName, elm, context ) {
			...
			if ( 1 === inGroup.length ) {
				...
				dependants = $( '[data-conditional-id]', inGroup ).filter(
					function() {
