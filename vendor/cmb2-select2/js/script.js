(function ($) {
	...
	$( '.cmb-repeatable-group' ).on(
		'cmb2_add_group_row_start', function (event, instance) {
			var $table  = $( document.getElementById( $( instance ).data( 'selector' ) ) );
			var $oldRow = $table.find( '.cmb-repeatable-grouping' ).last();

			$oldRow.find( '.pw_select2' ).each(
				function () {
	...
	$( '.cmb-repeatable-group' ).on(
		'cmb2_add_row', function (event, newRow) {
			...
			$( newRow ).find( '.pw_multiselect' ).each(
				function () {
					$( 'option:selected', this ).removeAttr( "selected" );
					$( this ).select2_sortable();
				}
			...
			$( newRow ).prev().find( '.pw_select' ).each(
				function () {
					$( this ).select2(
						{
							allowClear: true
			...
			// Reinitialise the field we previously destroyed
			$( newRow ).prev().find( '.pw_multiselect' ).each(
				function () {
					$( this ).select2_sortable();
				}
			);
	...
	$( '.cmb-repeatable-group' ).on(
		'cmb2_shift_rows_start', function (event, instance) {
			var groupWrap = $( instance ).closest( '.cmb-repeatable-group' );
			groupWrap.find( '.pw_select2' ).each(
				function () {
					$( this ).select2( 'destroy' );
				}
			);
	...
	$( '.cmb-repeatable-group' ).on(
		'cmb2_shift_rows_complete', function (event, instance) {
			var groupWrap = $( instance ).closest( '.cmb-repeatable-group' );
			groupWrap.find( '.pw_select' ).each(
				function () {
					$( this ).select2(
						{
							allowClear: true
			...
			groupWrap.find( '.pw_multiselect' ).each(
				function () {
					$( this ).select2_sortable();
				}
			);
