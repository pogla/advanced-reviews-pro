...
class CMB2_REST_Controller_Fields extends CMB2_REST_Controller_Boxes {
	...
	public function register_routes() {
		...
		register_rest_route(
			$this->namespace, '/' . $this->rest_base . '/(?P<cmb_id>[\w-]+)/fields/', array(
				array(
		...
		register_rest_route(
			$this->namespace, '/' . $this->rest_base . '/(?P<cmb_id>[\w-]+)/fields/(?P<field_id>[\w-]+)', array(
				array(
					'methods'             => WP_REST_Server::READABLE,
	...
	public function update_item( $request ) {
		...
		if ( ! $this->request['value'] ) {
			return new WP_Error(
				'cmb2_rest_update_field_error', __( 'CMB2 Field value cannot be updated without the value parameter specified.', 'cmb2' ), array(
					'status' => 400,
				)
			);
		}
	...
	public function modify_field_value( $activity ) {
		...
		if ( ! $this->request['object_id'] || ! $this->request['object_type'] ) {
			return new WP_Error(
				'cmb2_rest_modify_field_value_error', __( 'CMB2 Field value cannot be modified without the object_id and object_type parameters specified.', 'cmb2' ), array(
					'status' => 400,
				)
			);
		}
		...
		if ( ! $this->field ) {
			return new WP_Error(
				'cmb2_rest_no_field_by_id_error', __( 'No field found by that id.', 'cmb2' ), array(
					'status' => 403,
				)
			);
		}
